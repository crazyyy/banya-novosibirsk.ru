...
class PTB_CMB_Slider extends PTB_Extra_Base {
    ...
    public function action_them_themplate($id, $type, $args, $data = array(), array $languages = array()) {
        ...
        <div class="ptb_back_active_module_row">
            <div class="ptb_back_active_module_label">
                <label for="ptb_<?php echo $id ?>[captions]"><?php _e('Show Caption', 'ptb_extra') ?></label>
            </div>
            <div value="1" class="ptb_back_active_module_input ptb_back_text">
                <div class="ptb_custom_select">
                    <select id="ptb_<?php echo $id ?>[captions]" name="[<?php echo $id ?>][captions]">
                        <option <?php if (isset($data['captions']) && !$data['captions']): ?>selected="selected"<?php endif; ?> value="0"><?php _e('No', 'ptb_extra') ?></option>
                        <option <?php if (isset($data['captions']) && $data['captions']): ?>selected="selected"<?php endif; ?> value="1"><?php _e('Yes', 'ptb_extra') ?></option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <fieldset class="ptb_cmb_input">
            <ul id="<?php echo $meta_key ?>_options_wrapper" class="ptb_cmb_options_wrapper">
                ...
                <?php foreach ($values as $index => $v): ?>
                    ...
                    $title = isset($value['title']) && !empty($value['title'][$index]) ? esc_attr($value['title'][$index]) : '';
                    $description = isset($value['description']) && !empty($value['description'][$index]) ? esc_textarea($value['description'][$index]) : '';
                    ?>
        ...
        <div class="ptb_back_active_module_row">
            ...
            <div class="ptb_back_active_module_input">
                <div class="ptb-submission-small-input">
                    <input type="number" name="[<?php echo $id ?>][max]" value="<?php echo !empty($module['max'])?intval($module['max']):0 ?>" min="0"  id="ptb_<?php echo $id ?>_max" />
    ...
    public function ptb_submission_form($post_type, array $args, array $module, $post, $lang, $languages) {
        ...
        $size = PTB_Submissiion_Options::max_upload_size($module['size']);
        $max = !empty($module['max'])?intval($module['max']):false;
        if($max==0){
            $max = false;
        }
        $i = 0;
