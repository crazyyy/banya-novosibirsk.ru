...
class PTB_CMB_Gallery extends PTB_Extra_Base {
    ...
    public function action_public_themplate(array $args, array $data, $meta_data, $lang = false, $is_single = false) {
        ?>
        <?php if (!empty($meta_data) && isset($meta_data[$args['key']]) && !empty($meta_data[$args['key']]) && !empty($data['layout'])): ?>
            <?php
            $class = array();
    ...
    public function ptb_submission_save(array $m, $key, array $post_data, $post_id, $lng) {
        return array('url' => !empty($this->data[$key]['url']) ? $this->data[$key]['url'] : array(),
            'title' => !empty($this->data[$key]['title']) && !empty($this->data[$key]['title'][$lng]) ? $this->data[$key]['title'][$lng] : false
        );
    }
