 ...
 */
function fn_scx_get_op_name( $user_id = null ) {
 ...
 * @return void
 */
function fn_scx_update_op_caps() {
	global $ChatX;

	// Get current capabilities of operator users
	$current_caps = $ChatX->opts->getOption( 'op-caps' );
	...
	if( in_array( 'answer_visitors', $current_caps ) )
		$op_role->add_cap( 'scx_answer_visitor' ); // Add capability
	else
		$op_role->remove_cap( 'scx_answer_visitor' ); // Remove capability

	if( in_array( 'see_chat_logs', $current_caps ) )
		$op_role->add_cap( 'scx_see_logs' ); // Add capability
	else
		$op_role->remove_cap( 'scx_see_logs' ); // Remove capability

	if( in_array( 'manage_chat_options', $current_caps ) )
		$op_role->add_cap( 'scx_manage_chat_options' ); // Add capability
	else
		$op_role->remove_cap( 'scx_manage_chat_options' ); // Remove capability
 ...
 */
function fn_scx_get_role_names( $exclude = null ) {
	global $wp_roles;
 ...
 */
function fn_scx_get_op_cap_names() {
 ...
 */
function fn_scx_user_has_role( $roles, $user_id = null ) {
 ...
 */
function fn_scx_get_user_data_by_array( $user_id = null ) {
	global $ChatX;

	$site_logo = $ChatX->opts->getOption( 'site-logo' );
	$force_site_logo = $ChatX->opts->getOption( 'site-logo-force' );
	...
	if( is_user_logged_in() ) {
		...
		// Get name
		$data['name'] = ( defined( 'SCX_OP' ) ) ? fn_scx_get_op_name( $user->ID ) : $user->display_name;

		// Get other data
		$data['email'] = $user->user_email;
		$data['username'] = $user->user_login;

		if( current_user_can( 'scx_answer_visitor' ) && !empty( $force_site_logo ) ) {
			$data['avatar'] = $default_avatar;
		} else {
			$data['avatar'] = fn_scx_get_avatar( $user->user_email, null, $default_avatar );
		}
	// 
	// Visitor data
	// 
	} else {
		$unique_name = uniqid( scx__( 'guest-prefix' ), true );

		$data['name'] = fn_scx_ip_addr();
		$data['username'] = $unique_name;
		$data['email'] = '';
		$data['avatar'] = SCX_URL . '/assets/img/cx-default-avatar.png';
	}
 ...
 */
function fn_scx_get_avatar( $email, $avatar_size = 100, $default = null ) {
